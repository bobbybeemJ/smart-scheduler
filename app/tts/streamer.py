...
def check_engine_health() -> dict[str, bool]:
    """Boot-time check (Phase 6 calls this once at server startup and logs the result) - gives
    visibility into risk #2, since edge-tts is unofficial and can go down without notice."""
    result = {"edge_tts": False, "pyttsx3": False}

    try:
        asyncio.run(edge_tts_client.synthesize_bytes("test"))
