"""Gemini backend for extract_intent() - see app/llm/client.py for the provider dispatch and
app/llm/anthropic_backend.py for the alternative backend. Mockable via USE_MOCK_LLM (handled in
client.py, not here) so the rest of the pipeline can be built/debugged at zero token cost.

The regex fallbacks below exist specifically to compensate for gemini-flash-lite-latest (the
only Gemini model with usable free-tier quota - see project memory on the 20-requests/day caps
hit by gemini-3.5-flash/3.6-flash). Claude Haiku 4.5 (app/llm/anthropic_backend.py) does not
reproduce these failure modes in side-by-side testing (2026-07-27) - see that module's docstring
for the comparison. These fixes stay scoped to this file rather than being applied universally,
since they're a compensation for this specific model's weaknesses, not a general requirement."""
...
def extract_intent(transcript: str, condensed_state: Optional[dict] = None) -> TemporalExpression:
    ...
    if getattr(intent, "kind", None) == "simple_datetime":
        ...
        if raw_phrase is not None:
            stripped_phrase = strip_duration_mention(raw_phrase)
